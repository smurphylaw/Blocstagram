...
{
    NSArray *images = @{@"1.jpg": @"2.jpg"};
    // probably want to make the name more clear:
    // BLCComposeCommentView *heightTestCell = [[BLCComposeCommentView alloc] init];
    // BLCMedia *imageItem = [[BLCMedia alloc] init];

    // Since we have NSSize we would probably not want to name this ending in size, implies it has two floats
    CGFloat newImageHeight = [BLCMediaTableViewCell heightForMediaItem:images width:320];
    ...
    // so if you had a 320x320 image in imageItem..
    // and it's been set as item 0
    XCTAssertEqual([BLCMediaTableViewCell heightForMediaItem:images[0] width:650], 650);
    XCTAssertEqual([BLCMediaTableViewCell heightForMediaItem:images[0] width:200], 200);
