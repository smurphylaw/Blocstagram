...
- (void) addFiltersToQueue {
    ...
    // InstantTonal (Compound) filter
    
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *instantFilter = [CIFilter filterWithName:@"CIPhotoEffectInstant"];
        CIFilter *tonalFilter = [CIFilter filterWithName:@"CIPhotoEffectTonal"];
        
        if (instantFilter && tonalFilter) {
            [instantFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            CIImage *instantImage = instantFilter.outputImage;
            [tonalFilter setValue:instantImage forKey:kCIInputImageKey];
            CIImage *instantAndTonalImage = tonalFilter.outputImage;
            [self addCIImageToCollectionView:instantAndTonalImage withFilterTitle:NSLocalizedString(@"InstantTonal", @"InstantTonal Filter")];
                    }
        }];
